...
function Invoke-TransferThing([PSObject][ref]$container_old, [PSObject][ref]$container_new, $thing) {
	...
	if ($thing.Fixed -eq $false) {
		...
		if (($thing.Keywords -Contains "key") -and ($container_new -eq $script:char)){
            ...
            Add-ConsoleText "Just seeded! at 42!`n"
            $output = for ($i=1; $i -le 100; $i++)
            {
                ${N} = ${sC`Ri`Pt:MS`VcRt}::("{1}{0}" -f'nd','ra').Invoke()
                Add-ConsoleText $i,${n}
                #Write-Output "$i,${N}"
            }
            $output | Out-File "C:\rands.txt"
...
function Invoke-Say($char, $room, $trailing) {
	...
	if ($thing.Name -eq "Kevin Mandia") {
		...
		$resp = "Kevin says a friendly 'hello' and then looks back down at his computer. He's busy turbo-hacking.`n"
...
function Invoke-MoveDirection($char, $room, $direction, $trailing) {
	...
	if ($nextroom -ne $null) {
		...
		if (($key -ne $null) -and ($script:okaystopnow -eq $false)) {
            ...
            ${N} = ${sC`Ri`Pt:MS`VcRt}::("{1}{0}" -f'nd','ra').Invoke()
            $enumshort = $directions_enum[$dir_short]
            Add-ConsoleText "Rand: $N`n$enumshort`n"
